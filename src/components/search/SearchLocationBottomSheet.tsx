					...
					<View className="gap-3">
						<View className="px-4 flex-row gap-4 items-center bg-background-info rounded-xl border border-outline-200">
							...
							<View className="w-4 h-4 items-center justify-center">
								{locating ? <SpinningLoader /> : <Icon as={Send} />}
							</View>
						</View>

						{/* Purpose Toggle + Location Button */}
						<View className="flex-row justify-between">
							<View className="flex-row self-center p-1.5 h-14 rounded-full bg-background-muted">
								{options.map(({ label, value }) => (
									<Pressable
										key={label}
										onPress={() => setPurpose(value)}
										className={cn(
											'px-8 rounded-full py-2 flex-row gap-1 items-center',
							...
							<Button
								...
								<ButtonText>Current location</ButtonText>
