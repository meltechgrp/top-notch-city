...
export default function ListingCategory() {
  ...
  //     updateListing({ ...listing, category: categories[0]?.name });
  //   }
  // }, [categories]);
  const subs = useMemo(
  ...
  return (
    <>
            ...
            >
              {categories.map((c) => (
                <AnimatedPressable
                  key={c.id}
                  onPress={() => {
                    updateListing({ ...listing, category: c.name });
                  }}
            ...
            >
              {subs.map((b) => (
                <AnimatedPressable
                  key={b.id}
                  onPress={() => {
                    updateListing({ ...listing, subCategory: b.name });
                  }}
                  className={cn(
      ...
      <CurrencyPickerModal
        ...
        onSelect={(code) => updateListing({ ...listing, currency: code })}
      />
    </>
  );
}
